window.addEventListener('load',()=>{
  ...
  function clientEffect(){
    ...
    if(document.querySelector('#award_num').innerHTML!=maxNum[3]){
      let project=new ClientPlus('#project_num', maxNum[0])
      let counsel=new ClientPlus('#counsel_num', maxNum[1])
      let client=new ClientPlus('#client_num', maxNum[2])
      let award=new ClientPlus('#award_num', maxNum[3])
    }
...
class ClientPlus{ // 클라이언트 클래스

  constructor(selectPlus, maxNum){ // 대상, 최대값
    this.clientNum=document.querySelector(selectPlus)

    this.maxNum=maxNum
    
    this.timer=setInterval(()=>{this.clientPlus()}, 30)
